...
class KEFXIODriver extends Homey.Driver {
  
  async onInit() {
    this.log('KEF XIO Driver initialized');
  }
  ...
  async onPair(session: any) {
    ...
    session.setHandler('create_device', async (data: any) => {
      ...
      const device: any = {
        name: name || speakerInfo?.name || 'KEF XIO',
        data: {
          id: `kef-xio-${ip.replace(/\./g, '-')}`,
          model: 'kef-xio'
        },
        settings: {
          ip: ip,
