...
class KEFLS60Driver extends Homey.Driver {
  
  async onInit() {
    this.log('KEF LS60 Wireless Driver initialized');
  }
  ...
  async onPair(session: any) {
    ...
    session.setHandler('create_device', async (data: any) => {
      ...
      const device: any = {
        name: name || speakerInfo?.name || 'KEF LS60 Wireless',
        data: {
          id: `kef-ls60-${ip.replace(/\./g, '-')}`,
          model: 'kef-ls60'
        },
        settings: {
          ip: ip,
