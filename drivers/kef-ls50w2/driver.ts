...
class KEFLS50W2Driver extends Homey.Driver {
  
  async onInit() {
    this.log('KEF LS50 Wireless II Driver initialized');
  }
  ...
  async onPair(session: any) {
    ...
    session.setHandler('check_duplicate', async (data: any) => {
      ...
      const allDrivers = ['kef-ls50w2', 'kef-lsx2', 'kef-lsx2lt', 'kef-ls60', 'kef-xio'];
      ...
      for (const driverId of allDrivers) {
        try {
          ...
          for (const device of devices) {
            ...
            if (settings.ip === ip) {
              this.log(`[Check Duplicate] Found existing device: ${device.getName()} (${driverId}) with IP ${ip}`);
              return {
                isDuplicate: true,
    ...
    session.setHandler('test_connection', async (data: any) => {
      ...
      try {
        ...
        const speaker = new KEFSpeaker(ip, port || 80, (msg: string) => this.log(msg));
        ...
        this.log('[Test Connection] Speaker info received:', JSON.stringify(speakerInfo, null, 2));
    ...
    session.setHandler('create_device', async (data: any) => {
      ...
      const device: any = {
        name: name || speakerInfo?.name || 'KEF LS50 Wireless II',
        data: {
          id: `kef-ls50w2-${ip.replace(/\./g, '-')}`,
          model: 'kef-ls50w2'
        },
        settings: {
          ip: ip,
  ...
  async onRepair(session: any, device: any) {
    ...
    session.setHandler('test_connection', async (data: any) => {
      ...
      try {
        ...
        this.log('[Repair Test] Speaker info received:', JSON.stringify(speakerInfo, null, 2));
    ...
    session.setHandler('update_device', async (data: any) => {
      ...
      try {
        const speaker = new KEFSpeaker(data.ip, data.port || 80, (msg: string) => this.log(msg));
        const connected = await speaker.testConnection();
        ...
        const speakerInfo = await speaker.getSpeakerInfo();
        this.log('[Repair Update] Verified connection to:', speakerInfo.name, 'at', data.ip);
...
module.exports = KEFLS50W2Driver;