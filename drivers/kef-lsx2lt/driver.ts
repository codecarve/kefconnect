...
class KEFLSX2LTDriver extends Homey.Driver {
  
  async onInit() {
    this.log('KEF LSX II LT Driver initialized');
  }
  ...
  async onPair(session: any) {
    ...
    session.setHandler('create_device', async (data: any) => {
      ...
      const device: any = {
        name: name || speakerInfo?.name || 'KEF LSX II LT',
        data: {
          id: `kef-lsx2lt-${ip.replace(/\./g, '-')}`,
          model: 'kef-lsx2lt'
        },
        settings: {
          ip: ip,
...
module.exports = KEFLSX2LTDriver;