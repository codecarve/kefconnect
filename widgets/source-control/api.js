...
module.exports = {
  ...
  async setSource({ homey, body }) {
    try {
      const { deviceId, source } = body;
      if (!deviceId || !source) {
        throw new Error("Device ID and source are required");
      }
      ...
      // Call the capability listener to actually change the source
      if (device.onCapabilitySource) {
        await device.onCapabilitySource(source);
      } else if (device.triggerCapabilityListener) {
        await device.triggerCapabilityListener("source_input", source);
      }
